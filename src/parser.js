...
// Path absolute berdasarkan lokasi file ini, bukan cwd()
// Ini fix masalah workspace nyasar kalau bot dijalanin dari direktori lain
...
const FILE_DIRECTIVES = {
    ...
    'USER.md': 'Berikut adalah USER.md — ini adalah informasi tentang User, orang yang sedang berinteraksi denganmu. Gunakan ini untuk memahami konteks dan menyesuaikan responmu.',
...
export async function getSystemPrompt(forceRefresh = false, query = null) {
    ...
    const skills = [
        { name: 'TERMINAL', path: path.join(SKILLS_DIR, 'TERMINAL.md') },
        { name: 'BROWSER', path: path.join(SKILLS_DIR, 'BROWSER.md') },
        { name: 'FILEMANAGER', path: path.join(SKILLS_DIR, 'FILEMANAGER.md') },
    ];
...
// Baca nama User dari USER.md — fallback ke 'User' kalau tidak ketemu
export async function getOwnerName() {
    try {
        const content = await fs.readFile(path.join(WORKSPACE_DIR, 'USER.md'), 'utf-8');
        const match = content.match(/^-\s*Nama:\s*(.+)$/m);
